...
import { UploadCloud, Home, Tag, X, Edit } from "lucide-react";
...
export default function EditBrandPage() {
  ...
  useEffect(() => {
    const fetchBrand = async () => {
      try {
        ...
        if (brand.image) setImagePreview(brand.image);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await brandAPI.update(brandId, formData);
      ...
      console.error(error);
      setErrors({ form: "Có lỗi xảy ra khi cập nhật thương hiệu." });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="relative w-full min-h-screen p-6 bg-gray-50">
      ...
      <div className="flex flex-col gap-4 mb-6 md:flex-row md:justify-between md:items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Chỉnh Sửa Thương Hiệu</h1>
          <p className="text-gray-500">Quản lý thương hiệu của bạn</p>
        </div>
        <Breadcrumb
          items={[
        ...
        <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
          ...
          <div className="md:col-span-2">
            <Input
              ...
              placeholder="nhap-slug-thuong-hieu"
              error={errors.slug}
            />
            <p className="mt-1 text-sm text-gray-400">Slug tự động sinh từ tên, có thể chỉnh tay.</p>
          </div>
        </div>
