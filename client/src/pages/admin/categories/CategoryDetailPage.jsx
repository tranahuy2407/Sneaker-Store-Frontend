...
import { Home, Folder, X, Edit } from "lucide-react";
...
export default function CategoryDetailPage() {
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        ...
        const prodRes = await categoryAPI.getCategoryProducts(categoryId);
  ...
  if (!category) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <p className="text-lg text-red-500">Danh mục không tồn tại.</p>
      </div>
    );
  }
    ...
    <div className="relative w-full min-h-screen p-6 bg-gray-50">
      ...
      <div className="flex flex-col gap-4 mb-6 md:flex-row md:justify-between md:items-center">
        <div>
          ...
          <p className="text-gray-500">Thông tin chi tiết danh mục</p>
        </div>
        <Breadcrumb
          items={[
      ...
      <div className="p-6 space-y-6 bg-white rounded-lg shadow-md">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <h2 className="text-sm text-gray-500">Tên danh mục</h2>
          ...
          <div>
            ...
            <p className="text-gray-800">{new Date(category.created_at).toLocaleString()}</p>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button onClick={() => navigate(`/admin/categories/${category.id}/edit`)}>
            Chỉnh sửa
          </Button>
        </div>
      </div>
      ...
      {products.length > 0 && (
        <div className="p-6 mt-8 bg-white rounded-lg shadow-md">
          <h2 className="mb-4 text-lg font-bold text-gray-800">Sản phẩm thuộc danh mục</h2>
