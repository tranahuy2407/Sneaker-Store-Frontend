...
const CategoryPage = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const res = await categoryAPI.getCategoryProducts(slug, query);
      ...
      setProducts(mapped);
      setTotal(res.data?.data?.pagination?.total || items.length);

      const categories = items.flatMap((p) => p.categories || []);
      ...
      if (currentCat) {
        ...
        try {
          ...
          const catName = catRes.data?.data?.name;
      ...
      <div className="container px-5 mx-auto mt-4 mb-4">
        <Breadcrumb
          items={[
            ...
            { label: "Danh mục", icon: <List className="w-4 h-4" /> },
            { label: categoryName },
          ]}
        />
      </div>
